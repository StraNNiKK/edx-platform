...
class CertificateSearchTests(CertificateSupportTestCase):
    ...
    @ddt.unpack
    def test_search(self, user_query, expect_result, course_query=None):
        response = self._search(user_query, course_query)
        if expect_result:
    ...
    def _search(self, user_query, course_query=None):
        """Execute a search and return the response. """
        url = reverse("certificates:search") + "?user_query=" + user_query
        if course_query:
            url += '&course_id=' + course_query
