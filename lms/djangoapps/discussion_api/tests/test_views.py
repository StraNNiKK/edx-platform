...
class CommentViewSetListTest(DiscussionAPIViewTestMixin, ModuleStoreTestCase):
    ...
    def test_child_comments_count(self):
        ...
        response = self.client.get(self.url, {"thread_id": self.thread_id})
        expected_comments = [
            self.expected_response_comment(overrides={"id": "test_response_1", "children_count": 2}),
            self.expected_response_comment(overrides={"id": "test_response_2", "children_count": 3}),
        ]
        self.assert_response_correct(
            response,
            200,
